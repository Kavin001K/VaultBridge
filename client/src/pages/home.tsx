...
import { Lock, Upload, KeyRound, Shield, ArrowRight, RefreshCw, Mail, CheckCircle2, Clipboard, AlertTriangle, Github, ArrowUpRight, Zap, Globe, Code, Building, GraduationCap, Users, FileUp, } from "lucide-react";
...
export default function Home() {
  ...
  const focusAccess = () => { setTab("access"); setTimeout(() => { panelRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }); setTimeout(() => inpRef.current?.focus(), 400); }, 100); };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      <nav className="sticky top-0 z-50 border-b border-white/[0.04] bg-background/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button onClick={focusAccess} className="btn-ghost px-3 py-2 text-xs sm:text-sm">Access vault</button>
      ...
      {/* ─── HERO ─── */}
      <section className="relative pt-12 sm:pt-20 pb-4 px-4 sm:px-6">
        <div className="max-w-2xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }}>
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/[0.06] border border-primary/15 text-primary text-[11px] sm:text-xs font-medium tracking-wide mb-6">
              <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
              End-to-end encrypted file sharing
            </span>
          </motion.div>

          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1, ease: "easeOut" }}
            className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight leading-[1.08] mb-4 text-balance">
            Share files with<br /><span className="text-primary">complete privacy.</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.15, ease: "easeOut" }}
            className="text-sm sm:text-base text-zinc-400 max-w-md mx-auto mb-8 leading-relaxed">
            Encrypted in your browser. Access with a code. Gone after reading. No accounts, no tracking.
          </motion.p>

          {/* ─── SEGMENTED CONTROL ─── */}
          <motion.div ref={panelRef} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2, ease: "easeOut" }}>
            {/* Tab switcher */}
            <div className="inline-flex p-1 rounded-2xl bg-white/[0.03] border border-white/[0.06] mb-5">
              <button
                onClick={() => setTab("upload")}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${tab === "upload" ? "bg-primary text-primary-foreground shadow-sm" : "text-zinc-400 hover:text-white"}`}
              >
                <FileUp className="w-4 h-4" />Upload
              </button>
              <button
                onClick={() => setTab("access")}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${tab === "access" ? "bg-primary text-primary-foreground shadow-sm" : "text-zinc-400 hover:text-white"}`}
              >
                <KeyRound className="w-4 h-4" />Open vault
              </button>
            </div>
            ...
            <AnimatePresence mode="wait">
              {tab === "upload" ? (
                <motion.div key="upload" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}
                  ...
                  <div className="w-14 h-14 rounded-2xl bg-primary/[0.08] border border-primary/12 flex items-center justify-center mx-auto mb-5">
                    <Upload className="w-7 h-7 text-primary" />
                  </div>
                  <h3 className="text-xl font-semibold mb-2">Upload & encrypt files</h3>
                  <p className="text-sm text-zinc-400 mb-6 max-w-sm mx-auto leading-relaxed">
                    Drop any file up to 1 GB. AES-256-GCM encryption happens entirely in your browser. We never see your data.
                  </p>
                  <Link href="/upload">
                    <button className="btn-primary px-8 py-3 text-base">Start uploading<ArrowRight className="w-4 h-4" /></button>
                  </Link>
                  <div className="mt-6 flex flex-wrap items-center justify-center gap-4 text-[11px] text-zinc-500">
                    <span className="flex items-center gap-1.5"><Shield className="w-3 h-3 text-primary/60" />End-to-end encrypted</span>
                    <span className="flex items-center gap-1.5"><Zap className="w-3 h-3 text-primary/60" />Up to 1 GB per file</span>
                    <span className="flex items-center gap-1.5"><Globe className="w-3 h-3 text-primary/60" />Works on any device</span>
                  </div>
                </motion.div>
              ) : (
                <motion.div key="access" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}
                  className="card-accent p-5 sm:p-6 text-left">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-xl bg-primary/[0.1] border border-primary/20 flex items-center justify-center"><KeyRound className="w-5 h-5 text-primary" /></div>
                    <div>
                      <h3 className="text-base font-semibold text-white">Open a vault</h3>
                      <p className="text-xs text-zinc-500">Enter a code or paste a vault link.</p>
                    </div>
                  </div>
                  <div className="flex gap-2.5">
                    <div className="relative flex-1">
                      <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                      <input ref={inpRef} type="text" value={input} onChange={e => { setInput(e.target.value); if (err) setErr(null); }} onKeyDown={e => e.key === "Enter" && open(input)}
                    ...
                    <button onClick={() => open(input)} className="btn-primary px-6 shrink-0 text-sm font-medium">Open<ArrowRight className="w-3.5 h-3.5" /></button>
                  </div>
                  <div className="flex flex-wrap items-center gap-2 mt-3">
                    <button onClick={paste} className="btn-ghost px-3 py-1.5 text-[11px]"><Clipboard className="w-3 h-3" />Paste from clipboard</button>
                    {recent && <button onClick={() => open(recent)} className="btn-ghost px-3 py-1.5 text-[11px]"><RefreshCw className="w-3 h-3" />Recent vault</button>}
                    <Link href="/get-it-mailed"><button className="btn-ghost px-3 py-1.5 text-[11px] text-primary"><Mail className="w-3 h-3" />Email me a code</button></Link>
                  </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.3 }}
            className="flex flex-wrap items-center justify-center gap-6 sm:gap-10 mt-7 mb-2">
            {[{ v: stats?.activeVaultsCount ?? "—", l: "Active vaults" }, { v: stats?.totalVaultsCreated?.toLocaleString() ?? "—", l: "Total secured" }, { v: "AES-256", l: "Encryption" }].map(s => (
              <div key={s.l} className="text-center"><div className="text-2xl sm:text-3xl font-bold text-white tabular-nums">{s.v}</div><div className="text-xs text-zinc-500 mt-1">{s.l}</div></div>
            ))}
          </motion.div>
      ...
      <section className="border-t border-white/[0.04] section-padding px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10 sm:mb-14">
            ...
            <p className="text-sm sm:text-base text-zinc-500 max-w-md mx-auto">Three steps. Your files stay private at every stage.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6">
            {[
              { step: "01", icon: Upload, title: "Upload", desc: "Drop any file. Encryption happens in your browser using AES-256-GCM before a single byte leaves your device." },
              { step: "02", icon: KeyRound, title: "Share code", desc: "You get a short access code. Share it however you want — message, email, or in person. No accounts needed." },
              ...
              <motion.div key={s.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }}
                ...
                <div><div className="text-[10px] font-mono text-zinc-500 mb-1.5 tracking-wider">STEP {s.step}</div><h3 className="text-lg font-semibold mb-2">{s.title}</h3><p className="text-sm text-zinc-400 leading-relaxed">{s.desc}</p></div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-white/[0.04] section-padding px-4 sm:px-6">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10 sm:mb-14"><h2 className="text-2xl sm:text-3xl font-bold mb-3">Privacy by design</h2><p className="text-sm sm:text-base text-zinc-500">Every decision starts with one question: does this protect the user?</p></div>
          <div className="card-accent overflow-hidden">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-0">
              <div className="p-6 sm:p-8 border-b sm:border-b-0 sm:border-r border-white/[0.04]"><div className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-3">Feature</div>{comparisons.map((c, i) => <div key={i} className="py-2.5 text-sm font-medium text-white border-b border-white/[0.02] last:border-0">{c.label}</div>)}</div>
              <div className="p-6 sm:p-8 border-b sm:border-b-0 sm:border-r border-white/[0.04] bg-primary/[0.02]"><div className="text-xs font-medium text-primary uppercase tracking-wider mb-3">VaultBridge</div>{comparisons.map((c, i) => <div key={i} className="py-2.5 text-sm border-b border-white/[0.02] last:border-0 flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-primary shrink-0" /><span className="text-zinc-300">{c.us ? "Yes" : "No"}</span></div>)}</div>
              <div className="p-6 sm:p-8"><div className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-3">Traditional cloud</div>{comparisons.map((c, i) => <div key={i} className="py-2.5 text-sm border-b border-white/[0.02] last:border-0 flex items-center gap-2"><span className="text-zinc-600 w-4 h-4 flex items-center justify-center text-xs">—</span><span className="text-zinc-500">{c.them ? "Yes" : "No"}</span></div>)}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-white/[0.04] section-padding px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10 sm:mb-14"><h2 className="text-2xl sm:text-3xl font-bold mb-3">For everyone</h2><p className="text-sm sm:text-base text-zinc-500">Privacy tools that adapt to how you work.</p></div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {[{ i: Code, t: "Developers", d: "API keys, configs, env files" }, { i: Building, t: "Business", d: "Contracts, financial data" }, { i: GraduationCap, t: "Students", d: "Projects, research data" }, { i: Shield, t: "Journalists", d: "Sources, sensitive docs" }, { i: Users, t: "Anonymous", d: "Share without identity" }, { i: Globe, t: "Everyone", d: "Privacy is a right" }].map(uc => (
              <div key={uc.t} className="card-elevated p-4 sm:p-5 flex items-start gap-3"><div className="p-2 rounded-lg bg-white/[0.03] text-primary shrink-0"><uc.i className="w-4 h-4" /></div><div><h3 className="font-medium text-sm mb-0.5">{uc.t}</h3><p className="text-xs text-zinc-500">{uc.d}</p></div></div>
            ))}
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-white/[0.04] bg-background pt-14 sm:pt-20 pb-8 px-4 sm:px-6 safe-bottom">
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 sm:gap-12 mb-12">
          <div className="col-span-2 md:col-span-1"><div className="flex items-center gap-2.5 mb-4"><div className="w-8 h-8 rounded-lg bg-primary/10 border border-primary/20 flex items-center justify-center"><img src="/icon-192x192.png" alt="VB" className="w-4 h-4" /></div><span className="font-semibold text-white">VaultBridge</span></div><p className="text-xs text-zinc-500 leading-relaxed">Zero-knowledge encrypted file sharing. Built for privacy, not profit.</p></div>
