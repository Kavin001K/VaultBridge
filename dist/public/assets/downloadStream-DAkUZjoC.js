import { at as f } from "./index-CjvZq1Jz.js";
function c() {
...
async function E(o, m, v, t, u) {
  ...
  try {
    const n = await f(m), d = (r) => {
      const e = r.data;
      if (e?.fileId === o) switch (e.type) {
        case "DOWNLOAD_PROGRESS":
          u?.({ fileId: e.fileId, progress: e.progress, processedChunks: e.processedChunks, totalChunks: e.totalChunks, fileName: e.fileName });
          break;
    ...
    navigator.serviceWorker.addEventListener("message", d), navigator.serviceWorker.controller.postMessage({ type: "REGISTER_DOWNLOAD", fileId: o, keyBase64: n, chunks: v.map((r) => ({ downloadUrl: r.downloadUrl, index: r.index })), metadata: { name: t.name, size: t.size, type: t.type, fileId: t.fileId }, isCompressed: t.isCompressed || false }), await new Promise((r, e) => {
      const s = setTimeout(() => {
        ...
        l.data?.type === "DOWNLOAD_REGISTERED" && l.data.fileId === o && (clearTimeout(s), navigator.serviceWorker.removeEventListener("message", a), r());
      };
    ...
    return i.style.cssText = "position:absolute;width:0;height:0;border:none;visibility:hidden;", i.src = `/stream-download/${o}`, document.body.appendChild(i), await new Promise((r, e) => {
      const s = (a) => {
        a.data?.fileId === o && (a.data.type === "DOWNLOAD_COMPLETE" ? (navigator.serviceWorker.removeEventListener("message", s), r()) : a.data.type === "DOWNLOAD_ERROR" && (navigator.serviceWorker.removeEventListener("message", s), e(new Error(a.data.error))));
      };
